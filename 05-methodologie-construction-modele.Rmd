...
Pour ce faire, la librairie `caret` offre une fonction très utile `createDataPartition`.
```{r, echo=FALSE}
# train_ind <- c(caret::createDataPartition(y = data$target_duree, times = 1, p = .75, list = FALSE))

# y <- data[train_ind,]$target_duree
# X <- data[train_ind, -c("target_duree","target_meme_station")]

# y_test <- data[-train_ind,]$target_duree
# X_test <- data[-train_ind, -c("target_duree","target_meme_station")]
...
```{r, echo=TRUE}
# Reg lineaire avec data de reltation linéaire parfaite. Pas possible d'être parfait...
```
