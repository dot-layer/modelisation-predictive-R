...
ind_train <- sample(nrow(data),250000)
data <- fst::read.fst("data/data_preprocess.fst", as.data.table = T)[ind_train,]
```
...
Nous n'avons pas ce problème et avions laissé de côté un test assez gros pour en faire sortir un jeu de validation.
```{r, echo=T}
ind_test <- sample((1:(nrow(data)+length(ind_train)))[-ind_train],100000)
data_test <- fst::read.fst("data/data_preprocess.fst", as.data.table = T)[ind_test,]

ind_val <- sample(nrow(data_test),nrow(data_test)/2)
data_val <- data_test[ind_val,]
data_test <- data_test[-ind_val,]
```
