...
data <- fst::read.fst("data/data_preprocess.fst", as.data.table = T)[1:50000,]
...
Pour sélectionner un modèle parmi plusieurs, on reste encore fidèle à l'objectif ultime : minimiser l'erreur de généralisation.
Ceci passe généralement par un compromis entre le nombre de paramètres et la qualité de l'ajustement du modèle, ou directement par l'estimation de l'erreur de généralisation.
...
Avec nos données bixi, l'étape est très simple :
```{r, echo = T}
ind_test <- readRDS("data/models/ind_test.rds")
ind_test <- ind_test[1:10000]

data_test <- fst::read.fst("data/data_preprocess.fst", as.data.table = T)[15000:20000,]
data_test$moment_journee <- factor(data_test$moment_journee, labels=c("A","B","C","D"))

cols <- grep("start_q", colnames(data_test))
colnames(data_test)[cols] <- paste0("start_quartier_",1:length(cols))

data_test[,glm_pred := predict(object = glm_final, newx = model.matrix(f, data_test))]
