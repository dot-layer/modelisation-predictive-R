...
```{r, echo = T}
cols_moment <- grep(pattern = "moment", names(X_train)) # en numéro de colone
cols_sqg <- grep(pattern = "start_quartier_group", names(X_train))

cols_pair <- cbind(rep(cols_sqg,length(cols_moment)),
                   rep(cols_moment,length(cols_sqg)))

new_names <- apply(matrix(names(X_train)[c(cols_pair)], ncol = 2),
                   1,
                   paste0, collapse = ":::")

# Nouveau dataset avec les intéractions seulement -- à cbind(X_train,X_new)
X_new <- X_train
...
Il faut simplement lui fournir les données explicatives concernées.
...
ggplot(data.table(y = c(y_train,y_pred),
                  type=c(rep("true",length(y_train)), rep("pred",nrow(y_pred)))),
       aes(x=y, fill=type)) +
...
```{r, echo = T}
erreur <- colMeans(abs(predict.glmnet(glms, newx = as.matrix(X_val)) - y_val)) # Avec val!
