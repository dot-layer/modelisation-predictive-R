...
Dans cette section, nous verrons comment appliquer les concepts théoriques décrits dans la section précédente dans un contexte où nous voulons pas seulement entraîner un modèle, mais aussi l'uitiliser pour prédire de nouvelles observations dans un contexte de production. Une fois que le modèle est entraîné, nous devons répliquer ces prétraitements sur les nouvelles observations de la même manière que nous l'avons fait sur le jeu de données d'entraînement. Dans cette optique, il faudra garder une trace de ces prétraitements pour être en mesure de les effectuer sur le jeu de données d'entraînement et sur la donnée à prédire.

Pour se faire, nous proposons de faire 2 processus de prétraitements différents:

- `src/pretraitements/pretraitements-train.R`
- `src/pretraitements/pretraitements-predict.R`

Le premier servira à faire le prétraitement des données d'entraînement alors que le deuxième servira à faire le prétraitement des données à prédire lors de l'inférence. Les prochaines sections permettront de formaliser les différentes étapes clés à prévoir pour ces deux scénarios différents.

### Imputations de données manquantes

Dans le prétraitement des données d'entraînement, il faudra sauvegarder les valeurs d'imputation et tout autre objets permettant de faire l'imputation des données manquantes une fois rendu à l'inférence. Par exemple, il est possible de définir les valeurs d'imputations et de sauvegarder ces valeurs dans un fichier JSON:
...
Pour être en mesure de répliquer l'encode de données catégoriques, le principal défi est faire l'encodage *un-chaud*. La fonction `dummyVars` du package `caret` permet de réaliser assez facilement ce genre de transformation:
...
Pour les autres variables à encoder, il est généralement plus facile de répliquer les transformations faits lors de l'entraînement.
