...
La majorité des jeux de données réels sont témoins de données manquantes. Certains algorithmes ne sont pas en mesure de gérer les données manquantes par eux-mêmes. Cela nous oblige donc à traiter celles-ci et imputer le jeu de données. Les causes d'absence de données peuvent être relativement variées: bris de système, abstention, perte de données, etc. Il est généralement difficile de connaître la raison exacte, ce qui rend le traitement de données manquantes une étape importante. Avant même de trouver une méthode pour imputer ces données manquantes, il faut d'abord évaluer la quantité de données manquantes (voir fig \@ref(fig:missing)) et tenter de comprendre le mécanisme expliquant la non-réponse d'une donnée. 

```{r missing, echo=FALSE, fig.align='center', fig.cap="Pourcentage de données manquantes selon la variable du jeu de données BIXI."}

data.table(
  variable = names(data_bixi),
  missing_percentage = unlist(data_bixi[, lapply(.SD, function(x) sum(is.na(x))), .SDcols = 1:ncol(data_bixi)]/nrow(data_bixi))
) %>% 
  ggplot(aes(x = reorder(variable, -missing_percentage), y = missing_percentage)) +
    geom_bar(stat = "identity") +
    scale_x_discrete("Nom de la variable") +
    scale_y_continuous("% données manquantes", labels = percent, limits = c(NA, 0.01)) +
    coord_flip() + 
    theme_classic()
```
...
2. **Imputation par une mesure de centralité**: Utiliser la moyenne, la médianne ou le mode pour remplacer les données manquantes. Cette méthode est relativement simple, mais peu recommandée pour des variables ayant un pourcentage élevé de données manquantes (même pour MCAR). Cela diminue la variabilité et la corrélation entre les variables.
...
Maitenant, supposons que notre jeu de données est plus complexe et possède $p=10$ attributs au lieu d'un seul attribut. Les observations suivent toujours une loi uniforme où $x_1,...,x_n \stackrel{iid}{\sim} U([0,1]^{10})$. Combien d'observations en moyenne se trouveront dans l'intervalle $[0;0.1]^{10}$?
...
En termes plus simple, on cherche à trouver une combinaison linéaire des $p$ attributs qui permettra de maximiser la variance (information). Cette combinaison linéaire constituera notre première composante principale. Par la suite, on cherche une deuxième combinaison linéaire, qui aura comme contrainte d'être orthogonale à la première, qui maximise encore une fois la variance. On procède de cette manière jusqu'à temps qu'on juge avoir un nombre de composantes principales qui contiennent suffisament d'information et réduisent la dimensinnalité du jeu de données. À la figure \@ref(fig:acp) [@10.1371/journal.pone.0058552], on peut voir un exemple où avec seulement les 2 premières composantes principales, il est possible de reconstruire les données de manière intéressante avec 166 000 attributs génétiques différents. 
...
1. Il est important de normaliser (voir section \@ref(normalisation)) les données au préalable. Dans le cas contraire, certaines variables ayant des échelles importantes pourraient avoir l'impresssion d'apporter beaucoup de variance.
...
4. Pour plus d'informations sur le développement mathématique de la méthode, vous pouvez vous réferer au chapitre 10.2 du livre [@James:2014:ISL:2517747].
...
1. Positionnement multidimensionel: Trouver une représentation (avec une dimension inférieure à la dimension initiale) qui représente le mieux possible les distances entre les observations du jeu de données initial. Voir cette [page web](http://www.statsoft.com/textbook/multidimensional-scaling) pour plus de détails.
...
Dans certaines situations, il peut être utile de faire une séparation qui permet de s'assurer de la présence de certaines données dans chaque jeu de données. Pour faire ce genre de séparation, il est possible de faire un échantilloannage stratifiée sur certaines variables, ce qui assure la cohérence entre les deux jeux de données vis-à-vis ces variables. Cela peut être notamment utile dans un contexte de données débanlancées. Pour faciliter la mise en place de ce genre de séparation, il est possible d'utiliser la fonction `createDataPartition` du package `caret`:
...
Une fois que la séparation du jeu de données est faite, on peut désormais effectuer nos prétraitements via 2 processus différents. Un premier processus sera appliqué sur les données d'entraînement et un deuxième processus sera appliqué sur le jeu de données test (ou les données nouvelles données à prédire par notre modèle).

### Prétraitements en entraînement

Le processus de prétraitements en entraînement 

```{r workflow, echo=FALSE, fig.align='center', fig.cap="Processsing"}
knitr::include_graphics("static-files/preprocessing-workflow.png")
```



### Prétraitements en inférence

### Imputations de données manquantes

Dans le prétraitement des données d'entraînement, il faudra garder une trace des valeurs d'imputation ou tout autre objet permettant de faire l'imputation des données manquantes. Par exemple, il est possible de définir les valeurs d'imputations et de sauvegarder ces valeurs dans un fichier JSON en utilisant le package `jsonlite`:
...
write(jsonlite::toJSON("chemin/du/model/valeurs_imputations.json", pretty = TRUE))
```
...
Les fichiers JSON sont généralement faciles à traiter dans plusieurs logiciels et langagues différents. En R, il est également intéressant d'utiliser ce format qui se marie bien avec les listes. C'est donc un format que nous recommendons d'utiliser.
...
```{r, eval=FALSE, echo=TRUE}
variables_un_chaud <- c("weekend_flag")
# Il faut transformer les variables à encoder en factor d'abord
lapply(variables_un_chaud, function(var) data_bixi[, (var) := factor(get(var))])
objet_un_chaud <- dummyVars(" ~ .", data = data_bixi)
...
variables_un_chaud <- c("weekend_flag")
lapply(variables_un_chaud, function(var) data_inference[, (var) := factor(get(var))])
data_inference <- predict(objet_un_chaud, newdata = data_inference)
```

Le package `caret` offre plusieurs fonctionnalités intérressantes dans un contexte de modélisation. La fonction `dummyVars` est assez intuitve à utiliser et offre certaines options comme la possibilité de nommer les varibles indicatrices par un séparateur au choix (`sep`), comment gérer les nouvelles catégories (`na.action`), la possibilité d'enlever le niveau de base (`fullRank`), etc.


### Valeurs de normalisation

Pour ce qui est de la normalisation des attributs, il faut encore une fois garder une trace de ces valeurs pour ainsi normaliser adéquatement les données lors de l'inférence.
