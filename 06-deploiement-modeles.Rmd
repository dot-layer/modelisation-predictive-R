...
L'ultime étape pour donner vie à un modèle est de le mettre en production. Le cas d'utilisation générique considéré dans ce chapitre assume que l'usager veut 
...
```
pr <- plumber::plumb("deploiement/model.plumber/plumber_ini.R")
pr$run()
```
...
```
pr <- plumber::plumb("deploiement/model.plumber/pred_xgboost.R")
pr$run()
```
