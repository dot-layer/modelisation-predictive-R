...
L'importation de fichiers peut fonctionner à petite échelle. Pour éliminer des étapes manuelles, il est essentiel d'utiliser des BDs.
...
##### Données structurées
Les données structurées sont caractérisées par un modèle de données qui garanti une uniformité entre toutes les données, de façon à ce que la lecture et l'écriture de ces données sont facilitées et accélérées.

L'implantation la plus connue de données structurées est probablement les bases de données relationnelles constituées de tables disposées en colonnes et en rangées. Un langage comme sql Un modèle de données relie ces tables entre elles, de façon à pouvoir en extraire toutes les données à l'aide de requêtes qui joignent les tables entre elles. Le langage par excellence pour ce type de bases de données est le SQL.
